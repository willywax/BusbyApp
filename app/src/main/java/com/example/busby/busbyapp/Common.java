package com.example.busby.busbyapp;

/**
 * Created by NgocTri on 4/11/2016.
 */
public class Common {
    static final String LOGIN_URL = "http://guess-sa.gear.host/android_php/login.php";
    static final String NOTIFICATION_URL = "http://guess-sa.gear.host/android_php/notification.php";
    static final String NOTIFICATION_PUSH_URL = "http://guess-sa.gear.host/android_php/notificationPost.php";
    static final String IMAGE_PUSH_URL = "http://guess-sa.gear.host/android_php/ImagePost.php";
    static final String IMAGE_PULL = "http://guess-sa.gear.host/android_php/ImagePull.php";
    static final String IMAGE_PULL_GENERAL = "http://guess-sa.gear.host/android_php/ImagePullGeneral.php";
    static final String COMMENT_PUSH_URL = "http://guess-sa.gear.host/android_php/CommentPost.php";
    static final String COMMENT_PULL = "http://guess-sa.gear.host/android_php/CommentPull.php";
    static final String Site_URL = "http://guess-sa.gear.host/android_php/SearchSite.php";
    static final String Store_URL = "http://guess-sa.gear.host/android_php/SearchStore.php";
    static final String StoreID_PULL = "http://guess-sa.gear.host/android_php/StoreId.php";

    static final int RESULT_ERROR= -1;
    static final int RESULT_USER_EXISTS = 2;
}
